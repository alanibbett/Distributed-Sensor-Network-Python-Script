...
class Display:
	def __init__(self):
		#Check to see if we are using the SPI Port
		if SPI_PORT is not None:
			self.disp = Adafruit_SSD1306.SSD1306_128_64(rst=RST, dc=DC, spi=SPI.SpiDev(SPI_PORT, SPI_DEVICE, max_speed_hz=8000000))
		else:
			#no SPI configured so must be using i2c - so fire it up
		...
		self.screenSaverI = 0
		self.saverpic= globalsettings.SCREEN_SAVER_PIC
	...
	def displayScreenSaver(self):
		...
		if ( globalsettings.SCREEN_SAVER_TYPE == 0):
			...
			self.image = Image.open(self.saverpic).convert('1')
