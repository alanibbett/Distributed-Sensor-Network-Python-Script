...
class MenuSystem:
	def __init__(self):
			...
			globalsettings.display = self.display #put a reference to the display into the globals list
	...
	def checkButtons(self):
		...
		for button in self.buttons:
			if GPIO.input(button.GPIO_Pin)==GPIO.LOW:
				self.changedFlipFlop = True
				#if button 1 pressed
				...
				if (button.lable == "Button2"):
					...
					#print ("selected menu value %s"%(globalsettings.selectedMenu))
					self.display.drawTitle(self.titles[globalsettings.selectedMenu])
